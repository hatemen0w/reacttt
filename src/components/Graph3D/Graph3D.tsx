...
//     if (id){
//     this.canvas = document.getElementById(id) as 
//     HTMLCanvasElement;
//     //as необходимо использовать как можно меньше
//     }else{
//     this.context = this.canvasgetContext(‘2D’) as 
//     CanvasRenderingContext2D;
//     this.WIN = WIN;
//     …
//     }

//     //каждый метод класса должен иметь типа возвращаемого значения
//     xs(n:number):number {…}
//     polygon(points: Omit < Point, ‘z’>[], color = ‘#00ff00’): void{
//     //…
//     }
//     }
//     Point.ts
...
//     В math3D сделать enum3D в который записать все преобразования
...
//     // И так со всеми
//     math.calcDistance(surface, WIN.CAMERA, Edistance.distance){}
...
const Graph3D = () => {
    ...
    const moveStep = 4;
    let canMove = false;
    let rotateXT = math3D.getOneT();
    let rotateYT = math3D.getOneT();
        ...
        if (canMove) {
            switch (mouseButton) {
                case 0:
                    ...
                    break;
                case 1:
                    const T1 = math3D.getMoveT({ dy: graph.sy(event.movementY) * moveStep });
                    scene.forEach(surface => {
                        surface.points.forEach(point =>
                    ...
                    break;
                case 2:
                    const T2 = math3D.getMoveT({ dx: graph.sx(event.movementX) * moveStep })
                    scene.forEach(surface => {
                        surface.points.forEach(point =>
